...
class FbxMesh extends FbxGeometry {
  ...
  List<Vector3?> get deformedPoints {
    _deformedPoints ??= List<Vector3>.generate(points?.length ?? 0 ,(index) => Vector3.zero(),);
  ...
  void updateDisplayMesh() {
    ...
    for (var pi = 0, len = pts?.length ?? -1; pi < len; ++pi) {
      for (var vi = 0; vi < (disp.pointMap[pi]?.length ?? -1); ++vi) {
  ...
  void computeLinearBlendSkinning(FbxNode meshNode) {
    ...
    for (var pi = 0, len = points?.length ?? -1; pi < len; ++pi) {
  ...
  void generateClusterMap() {
    clusterMap = List.generate(points?.length ?? 0, (index) => null);
  ...
  void generateDisplayMeshes() {
    ...
    disp.pointMap = List<List<int>?>.generate(points!.length, (index) => null);
    ...
    if (_clusters.isNotEmpty) {
      ...
      for (var ci = 0, len = _clusters.length; ci < len; ++ci) {
        ...
        for (var xi = 0, numPts = cluster.indexes?.length ?? -1; xi < numPts; ++xi) {
          final weight = cluster.weights![xi];
          final pi = cluster.indexes![xi];

          if (disp.pointMap[pi] != null) {
            for (var vi = 0, nv = disp.pointMap[pi]?.length ?? -1; vi < nv; ++vi) {
      ...
      int j = 0;
      for (int index = 0; index < (uvs?.indexArray.length ?? 0); index++) {
  ...
  void _loadPoints(FbxElement e) {
    ...
    points = List.generate(p.length ~/ 3,(index) => Vector3.zero(),);
  ...
  void _loadNormals(FbxElement e) {
    ...
    for (final c in e.children) {
      ...
      if (c.id == 'MappingInformationType') {
        ...
        normals.data = List<Vector3>.generate(p.length ~/ 3,(index) => Vector3.zero(),);
        for (var i = 0, j = 0, len = p.length; i < len; i += 3) {
  ...
  void _loadUvs(FbxElement e) {
    ...
    for (final c in e.children) {
      ...
      if (c.id == 'MappingInformationType') {
        ...
        uvs.data = List<Vector2>.generate(p.length ~/ 2,(index) => Vector2.zero(),);
      ...
      //TODO UVIndex
        ...
        uvs.indexArray = List<int>.generate(p.length,(index) => 0,);
